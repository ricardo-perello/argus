...
This is the same missing layer described in the main documentation: WARP wants
the IOP-to-IA/iBCS compiler path that turns oracle commitments and openings into
an interactive argument in a principled way.

The implementation therefore handles oracle-opening material in an ad hoc,
protocol-local way. Roots, query answers, and opening data flow through ordinary
channel messages or local WARP structures instead of being generated by a
general BCS layer. This keeps the port useful as a case study, but the clean
long-term design is an iBCS/BCS compiler that owns commitment openings the same
way `spongefish-dsfs` owns sponge transcript operations.
